...
export default function RegisterFlow() {
  ...
  const prefillIdCard = searchParams.get('idCard') ?? ''
  const { login } = useAuth()
      ...
      <form ref={formRef} onSubmit={handleSubmit} className="p-5 space-y-5 pb-10">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-5 space-y-4">
          ...
          <Field label="เลขบัตรประชาชน * (Username)" name="id_card"
            ...
            note="13 หลัก — ใช้เข้าสู่ระบบ" errMsg={fieldErr.id_card} defaultValue={prefillIdCard} />
        ...
        <button type="submit" disabled={saving}
          ...
          {saving ? <><RefreshCw className="w-6 h-6 animate-spin" />กำลังสมัคร...</> : <><Check className="w-6 h-6" />ยืนยันสมัครสมาชิก</>}
        </button>

        <p className="text-center text-gray-400 text-sm">
          มีบัญชีแล้ว?{' '}
          <button type="button" onClick={() => navigate('/signin')} className="text-emerald-600 font-bold">เข้าสู่ระบบ</button>
        </p>
      </form>
    </div>
  )
