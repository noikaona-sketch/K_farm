...
export default function FieldSeedBooking() {
  ...
  const [history, setHistory] = useState<FieldBookingHistory[]>([])
  ...
  const load = async () => {
    ...
    try {
      ...
      const [farmerRes, profileRes, varietyRes, supplierRes, locRes, slotRes, historyRes] = await Promise.all([
        ...
        supabase.from('seed_bookings').select('id,booking_no,booking_date,farmer_name,farmer_phone,total_amount,status,created_by,created_by_name,booking_source,created_at').eq('created_by', user?.id ?? '').order('created_at', { ascending: false }).limit(20),
      ])
      ...
      setFarmers((farmerRes.data ?? []).filter((f: any) => ['approved', 'active', 'farmer', 'member'].includes(String(f.status ?? 'approved'))).map((f: any) => {
        const p = profileMap.get(String(f.profile_id ?? ''))
        return { id: String(f.id), name: String(f.full_name ?? p?.full_name ?? f.name ?? '-'), phone: String(f.phone ?? p?.phone ?? ''), idCard: String(f.id_card ?? p?.id_card ?? ''), district: String(f.district ?? ''), village: String(f.village ?? '') }
      }))
      const supplierMap = new Map((supplierRes.data ?? []).map((s: any) => [String(s.id), String(s.supplier_name ?? '-')]))
      setItems((varietyRes.data ?? []).map((v: any) => {
        const price = Number(v?.sell_price_per_bag ?? 0) || Number(v?.price ?? 0)
        return { id: String(v.id), varietyId: String(v.id), name: String(v.variety_name ?? '-'), supplier: supplierMap.get(String(v.supplier_id ?? '')) ?? '-', price }
      }))
      ...
      setHistory((historyRes.data ?? []).map((r: any) => ({
        ...
        created_by_name: String(r.created_by_name ?? 'ทีมภาคสนาม'),
        booking_source: String(r.booking_source ?? 'field'),
      })))
  ...
  const add = (item: SeedItem) => setCart((prev) => {
    const old = prev.find((x) => x.id === item.id)
    return old ? prev.map((x) => x.id === item.id ? { ...x, qty: x.qty + 1 } : x) : [...prev, { ...item, qty: 1 }]
  })
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => void load()} className="bg-white border rounded-xl p-2"><RefreshCw className={`w-5 h-5 ${loading ? 'animate-spin' : ''}`} /></button>
      </div>

      <div className="bg-emerald-600 text-white rounded-3xl p-5 shadow-lg">
        <div className="flex items-center gap-3"><Package className="w-8 h-8" /><div><h1 className="text-xl font-bold">จองเมล็ดพันธุ์</h1><p className="text-sm text-emerald-100">สำหรับทีมภาคสนาม</p></div></div>
      </div>
