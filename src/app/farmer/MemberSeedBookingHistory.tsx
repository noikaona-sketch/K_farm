...
import { ArrowLeft, CalendarCheck, ChevronDown, ChevronUp, MapPin, Package, RefreshCw, UserCheck, Wallet } from 'lucide-react'
...
export default function MemberSeedBookingHistory() {
  ...
  const load = async () => {
    ...
    try {
      ...
      let q = supabase
        .from('seed_bookings')
        .select('id,booking_no,booking_date,receive_date,pickup_date,pickup_time,pickup_location_name,total_amount,deposit_amount,status,farmer_id,booking_source,created_by_name')
      ...
      setRows((data ?? []).map((r: any) => ({
        ...
        booking_source: String(r.booking_source ?? 'member'),
