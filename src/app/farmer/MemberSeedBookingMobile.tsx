...
export default function MemberSeedBookingMobile() {
  ...
  const debt = Math.max(total - Number(deposit || 0), 0)
  ...
  const load = async () => {
    ...
    try {
      ...
      const [varietyRes, lotRes, locRes, slotRes] = await Promise.all([
        ...
        supabase.from('seed_stock_lots').select('id,variety_id,variety_name,lot_no,supplier_name,quantity_balance,status').gt('quantity_balance', 0).neq('status', 'inactive').order('created_at', { ascending: true }),
      ...
      setItems((lotRes.data ?? []).map((l: any) => { const v: any = vmap.get(String(l.variety_id)); return { id: String(l.id), varietyId: String(l.variety_id ?? ''), name: String(l.variety_name ?? v?.variety_name ?? '-'), lotNo: String(l.lot_no ?? '-'), supplier: String(l.supplier_name ?? '-'), price: Number(v?.sell_price_per_bag ?? v?.price ?? 0), stock: Number(l.quantity_balance ?? 0) } }))
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (totalQty > remain) throw new Error(`รอบรับนี้เหลือ ${remain} ถุง`)
      const bookingNo = genBookingNo()
      const { data: booking, error: bErr } = await supabase.from('seed_bookings').insert({ booking_no: bookingNo, booking_date: today(), receive_date: selectedSlot.pickup_date, farmer_id: user?.id ?? null, farmer_name: user?.name ?? 'สมาชิก', farmer_phone: user?.phone ?? '', total_amount: total, deposit_amount: Number(deposit || 0), balance_amount: debt, status: 'pending', payment_status: 'unpaid', pickup_location_id: selectedLocation.id, pickup_slot_id: selectedSlot.id, pickup_location_name: selectedLocation.name, pickup_date: selectedSlot.pickup_date, pickup_time: selectedSlot.pickup_time, note }).select('id').single()
      ...
      setCart([]); setDeposit('0'); setNote(''); setOk(`จองสำเร็จ เลขที่ ${bookingNo}`); await load()
    } catch (e) { setError(e instanceof Error ? e.message : 'จองไม่สำเร็จ') } finally { setSaving(false) }
  }

  return <div className="min-h-screen bg-gray-50 p-4 space-y-4"><div className="flex items-center justify-between"><button onClick={() => nav('/farmer')} className="bg-white border rounded-xl p-2"><ArrowLeft className="w-5 h-5" /></button><button onClick={() => nav('/farmer/seed-booking-history')} className="bg-white border rounded-xl px-3 py-2 text-sm flex gap-1"><History className="w-4 h-4" />ประวัติ</button></div><div className="bg-emerald-600 text-white rounded-3xl p-5 shadow-lg"><div className="flex items-center gap-3"><Package className="w-8 h-8" /><div><h1 className="text-xl font-bold">จองเมล็ดพันธุ์</h1><p className="text-sm text-emerald-100">เลือกสินค้าแบบ Marketplace</p></div></div></div>{ok && <div className="rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 p-3 text-sm">{ok}</div>}{error && <div className="rounded-xl bg-red-50 border border-red-200 text-red-700 p-3 text-sm">{error}</div>}{loading ? <div className="text-center text-gray-400 py-8"><RefreshCw className="w-6 h-6 mx-auto animate-spin" />กำลังโหลด...</div> : <div className="grid grid-cols-1 gap-3">{items.map((x) => <div key={x.id} className="bg-white rounded-2xl border p-4"><div className="flex justify-between gap-3"><div><div className="font-bold text-gray-900">{x.name}</div><div className="text-xs text-gray-500">{x.supplier} | LOT {x.lotNo}</div><div className="text-xs text-gray-500">คงเหลือ {money(x.stock)} ถุง</div></div><div className="text-right"><div className="font-bold text-emerald-700">{money(x.price)}</div><div className="text-xs text-gray-400">บาท/ถุง</div></div></div><button onClick={() => add(x)} className="mt-3 w-full rounded-xl bg-emerald-600 text-white py-2 font-bold flex justify-center gap-2"><Plus className="w-4 h-4" />เพิ่มลงตะกร้า</button></div>)}</div>}<div className="bg-white rounded-2xl border p-4 space-y-3"><div className="font-bold flex gap-2"><ShoppingCart className="w-5 h-5" />ตะกร้าจอง</div>{cart.length === 0 ? <div className="text-center text-gray-400 py-4">ยังไม่มีสินค้า</div> : cart.map((x) => <div key={x.id} className="border rounded-xl p-3"><div className="font-semibold">{x.name}</div><div className="flex items-center justify-between mt-2"><div className="flex items-center gap-2"><button onClick={() => x.qty <= 1 ? remove(x.id) : setQty(x.id, x.qty - 1)} className="border rounded-lg p-1"><Minus className="w-4 h-4" /></button><span className="font-bold w-8 text-center">{x.qty}</span><button onClick={() => setQty(x.id, x.qty + 1)} className="border rounded-lg p-1"><Plus className="w-4 h-4" /></button></div><b>{money(x.qty * x.price)} บาท</b></div></div>)}<div className="border-t pt-3 space-y-2"><select value={locationId} onChange={(e) => { setLocationId(e.target.value); setSlotId('') }} className="w-full border rounded-xl p-3 bg-white"><option value="">เลือกจุดรับสินค้า</option>{locations.map((l) => <option key={l.id} value={l.id}>{l.name}</option>)}</select>{selectedLocation?.address && <div className="text-xs text-gray-500">{selectedLocation.address}</div>}<select value={slotId} onChange={(e) => setSlotId(e.target.value)} disabled={!locationId} className="w-full border rounded-xl p-3 bg-white"><option value="">เลือกรอบรับ</option>{availableSlots.map((s) => <option key={s.id} value={s.id}>{s.pickup_date} {s.pickup_time} | เหลือ {money(Number(s.capacity_qty || 0) - Number(s.booked_qty || 0))}</option>)}</select><input type="number" value={deposit} onChange={(e) => setDeposit(e.target.value)} placeholder="เงินมัดจำ" className="w-full border rounded-xl p-3" /><textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="หมายเหตุ" className="w-full border rounded-xl p-3" /><div className="flex justify-between text-lg"><span>รวม</span><b>{money(total)} บาท</b></div><div className="flex justify-between text-red-600"><span>ยอดค้าง</span><b>{money(debt)} บาท</b></div><button disabled={saving || cart.length === 0} onClick={submit} className="w-full rounded-xl bg-emerald-600 disabled:bg-gray-300 text-white py-3 font-bold flex justify-center gap-2"><CalendarCheck className="w-5 h-5" />{saving ? 'กำลังจอง...' : 'ยืนยันจอง'}</button></div></div></div>
}
