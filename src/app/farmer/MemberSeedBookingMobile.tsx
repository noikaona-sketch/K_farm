...
export default function MemberSeedBookingMobile() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [varietyRes, supplierRes, locRes, slotRes] = await Promise.all([
      ...
      setItems((varietyRes.data ?? []).map((v: any) => { const price = Number(v?.sell_price_per_bag ?? 0) || Number(v?.price ?? 0); return { id: String(v.id), varietyId: String(v.id), name: String(v.variety_name ?? '-'), lotNo: '-', supplier: supplierMap.get(String(v.supplier_id ?? '')) ?? '-', price, stock: 0 } }))
