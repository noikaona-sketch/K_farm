...
export default function AdminSeedBooking() {
  ...
  const load = async () => {
    ...
    try {
      if (!isSupabaseReady || !supabase) {
        setFarmers(MOCK_FARMERS); setLots(MOCK_LOTS); setBookings([])
      ...
      const [varietyRes, supplierRes, bookingRes, locationRes, slotRes] = await Promise.all([
      ...
      setLots((varietyRes.data ?? []).map((v: any) => {
        const supplierId = String(v.supplier_id ?? '')
        const price = Number(v?.sell_price_per_bag ?? 0) || Number(v?.price ?? 0)
        return { id: String(v.id), supplierId, supplierName: supplierMap.get(supplierId) ?? '-', varietyId: String(v.id), varietyName: String(v.variety_name ?? '-'), lotNo: '', balance: 0, price }
      }))
  ...
  const bookingSummary = useMemo(() => { const totalAmount = filteredBookings.reduce((sum, b) => sum + Number(b.total_amount || 0), 0); const deposit = filteredBookings.reduce((sum, b) => sum + Number(b.deposit_amount || 0), 0); const pending = filteredBookings.filter((b) => b.status !== 'converted' && b.status !== 'cancelled').length; return { count: filteredBookings.length, totalAmount, deposit, debt: Math.max(totalAmount - deposit, 0), pending } }, [filteredBookings])
  const summaryByStatus = useMemo(() => { const rows = new Map<string, { status: string; count: number; total: number }>(); filteredBookings.forEach((b) => { const key = b.status || 'pending'; const prev = rows.get(key) ?? { status: key, count: 0, total: 0 }; rows.set(key, { ...prev, count: prev.count + 1, total: prev.total + Number(b.total_amount || 0) }) }); return Array.from(rows.values()) }, [filteredBookings])
  ...
  const submitBooking = async () => {
    ...
    try {
      ...
      if (!isSupabaseReady || !supabase) {
        setBookings((prev) => [{ id: `mock-${Date.now()}`, booking_no: bookingNo, booking_date: bookingDate, receive_date: receiveDate, farmer_id: selectedFarmer.id, farmer_name: selectedFarmer.name, farmer_phone: selectedFarmer.phone, total_amount: total, deposit_amount: 0, status: 'pending', note }, ...prev])
        clearBooking(); setOk(`สร้างใบจอง ${bookingNo} แล้ว`); return
      }
      const { data: booking, error: bookingErr } = await supabase.from('seed_bookings').insert({ booking_no: bookingNo, booking_date: bookingDate, receive_date: receiveDate, farmer_id: selectedFarmer.id, farmer_name: selectedFarmer.name, farmer_phone: selectedFarmer.phone, total_amount: total, deposit_amount: 0, balance_amount: debt, status: 'pending', payment_status: 'unpaid', pickup_location_id: selectedLocation.id, pickup_slot_id: selectedSlot.id, pickup_location_name: selectedLocation.name, pickup_date: selectedSlot.pickup_date, pickup_time: selectedSlot.pickup_time, note }).select('id').single()
      if (bookingErr) throw new Error(bookingErr.message)
      const items = cart.map((item) => ({ booking_id: booking.id, lot_id: null, lot_no: null, variety_id: item.varietyId, variety_name: item.varietyName, quantity: item.qty, sell_price_per_bag: item.price, total_amount: item.qty * item.price, reserved_quantity: 0, status: 'pending' }))
      ...
      await load(); clearBooking(); setOk(`สร้างใบจอง ${bookingNo} แล้ว`)
  ...
  const convertToPOS = async (booking: BookingRow) => {
    ...
    try {
      ...
      if (itemErr) throw new Error(itemErr.message)
      const items = (data ?? []).map((item: any) => ({ id: String(item.lot_id ?? item.variety_id ?? ''), supplierName: '', varietyId: String(item.variety_id ?? ''), varietyName: String(item.variety_name ?? '-'), lotNo: String(item.lot_no ?? ''), balance: Number(item.quantity_balance ?? 0), price: Number(item.sell_price_per_bag ?? 0), qty: Number(item.quantity ?? 0) }))
      ...
      window.localStorage.setItem('seed_booking_to_pos', JSON.stringify({ bookingId: booking.id, bookingNo: booking.booking_no, farmerId: booking.farmer_id, farmerName: booking.farmer_name, farmerPhone: booking.farmer_phone, depositAmount: booking.deposit_amount, items }))
      window.location.href = '/admin/seed-invoice'
    } catch (e) { setError(e instanceof Error ? e.message : 'แปลงเป็นขายไม่สำเร็จ') } finally { setSaving(false) }
  }

  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3"><div><h1 className="text-xl font-bold text-gray-900">POS จองเมล็ดพันธุ์</h1><div className="flex items-center gap-1.5 mt-0.5 text-sm">{isSupabaseReady ? <><Wifi className="w-3.5 h-3.5 text-emerald-600" /><span className="text-emerald-600">Booking + Cart</span></> : <><WifiOff className="w-3.5 h-3.5 text-amber-500" /><span className="text-amber-600">Mock data</span></>}</div></div><button onClick={() => void load()} className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-xl text-sm"><RefreshCw className="w-4 h-4" />รีโหลด</button></div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        <div className="xl:col-span-2"><SeedPOSProductGrid lots={filteredLots} suppliers={suppliers} supplierFilter={supplierFilter} productSearch={productSearch} onSupplierFilterChange={setSupplierFilter} onProductSearchChange={setProductSearch} onAddToCart={(lot) => setCart((prev) => addLotToCart(prev, lot))} /></div>
        <AdminSeedBookingCart cart={cart} farmers={farmers} selectedFarmerId={selectedFarmerId} selectedLocationId={selectedLocationId} selectedSlotId={selectedSlotId} pickupLocations={pickupLocations} pickupSlots={pickupSlots} bookingDate={bookingDate} receiveDate={receiveDate} note={note} saving={saving} total={total} debt={debt} onCartChange={setCart} onFarmerChange={setSelectedFarmerId} onLocationChange={setSelectedLocationId} onSlotChange={setSelectedSlotId} onBookingDateChange={setBookingDate} onReceiveDateChange={setReceiveDate} onNoteChange={setNote} onClear={clearBooking} onSubmit={submitBooking} />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-5 gap-3"><div className="bg-white border rounded-2xl p-4"><div className="text-xs text-gray-500">จำนวนใบจอง</div><div className="text-xl font-bold">{bookingSummary.count}</div></div><div className="bg-white border rounded-2xl p-4"><div className="text-xs text-gray-500">รอดำเนินการ</div><div className="text-xl font-bold text-amber-600">{bookingSummary.pending}</div></div><div className="bg-white border rounded-2xl p-4"><div className="text-xs text-gray-500">ยอดจองรวม</div><div className="text-xl font-bold">{fmtMoney(bookingSummary.totalAmount)}</div></div><div className="bg-white border rounded-2xl p-4"><div className="text-xs text-gray-500">เงินมัดจำ</div><div className="text-xl font-bold text-emerald-600">{fmtMoney(bookingSummary.deposit)}</div></div><div className="bg-white border rounded-2xl p-4"><div className="text-xs text-gray-500">ยอดค้าง</div><div className="text-xl font-bold text-red-600">{fmtMoney(bookingSummary.debt)}</div></div></div>
      <div className="bg-white rounded-2xl border p-4 space-y-3"><div className="flex items-center justify-between gap-3 flex-wrap"><div className="font-bold">สรุปประวัติจอง</div><div className="relative w-full md:w-96"><Search className="w-4 h-4 absolute left-3 top-3 text-gray-400" /><input value={historySearch} onChange={(e) => setHistorySearch(e.target.value)} placeholder="ค้นหาชื่อสมาชิก / เบอร์ / เลขจอง / สถานะ" className="w-full border rounded-xl pl-9 pr-3 py-2" /></div></div>{summaryByStatus.length > 0 && <div className="flex flex-wrap gap-2 text-xs">{summaryByStatus.map((s) => <span key={s.status} className="px-3 py-1 rounded-full bg-gray-100 text-gray-700">{s.status}: {s.count} ใบ / {fmtMoney(s.total)}</span>)}</div>}</div>
      <div className="bg-white rounded-2xl border overflow-x-auto"><table className="w-full text-sm"><thead><tr className="bg-gray-50 text-xs text-gray-500"><th className="text-left p-3">เลขจอง</th><th className="text-left p-3">วันที่จอง</th><th className="text-left p-3">นัดรับ</th><th className="text-left p-3">สมาชิก</th><th className="text-right p-3">ยอดจอง</th><th className="text-right p-3">มัดจำ</th><th className="text-right p-3">ค้าง</th><th className="text-center p-3">สถานะ</th><th className="text-left p-3">หมายเหตุ</th><th className="text-center p-3">จัดการ</th></tr></thead><tbody>{loading ? <tr><td colSpan={10} className="p-6 text-center text-gray-400">กำลังโหลด...</td></tr> : filteredBookings.length === 0 ? <tr><td colSpan={10} className="p-6 text-center text-gray-400">ยังไม่มีรายการจอง</td></tr> : filteredBookings.map((b) => <tr key={b.id} className="border-t"><td className="p-3 font-mono">{b.booking_no}</td><td className="p-3">{b.booking_date}</td><td className="p-3">{b.receive_date}</td><td className="p-3"><div>{b.farmer_name}</div><div className="text-xs text-gray-400">{b.farmer_phone}</div></td><td className="p-3 text-right">{fmtMoney(b.total_amount)}</td><td className="p-3 text-right text-emerald-600">{fmtMoney(b.deposit_amount)}</td><td className="p-3 text-right text-red-600">{fmtMoney(Math.max(Number(b.total_amount || 0) - Number(b.deposit_amount || 0), 0))}</td><td className="p-3 text-center">{b.status}</td><td className="p-3">{b.note || '-'}</td><td className="p-3 text-center"><button type="button" disabled={saving || b.status === 'converted'} onClick={() => void convertToPOS(b)} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-600 text-white text-xs font-semibold disabled:bg-gray-300"><ShoppingCart className="w-3 h-3" />แปลงเป็นขาย</button></td></tr>)}</tbody></table></div>
    </div>
  )
}
