...
type Member = { id: string; profileId?: string; name: string; phone: string; district?: string; village?: string; lat?: number; lng?: number }
type Variety = { id: string; name: string }
type Booking = { id: string; farmer_id: string; profile_id?: string; member_name: string; member_phone: string; variety_id: string; variety_name: string; quantity_kg: number; pickup_date: string; pickup_note?: string; status: BookingStatus; booked_by_role?: string; lat?: number; lng?: number; district?: string; village?: string }
...
const MOCK_MEMBERS: Member[] = [{ id: 'mock-f1', profileId: 'mock-p1', name: 'สมชาย ใจดี', phone: '0812345678', district: 'สำโรง', village: 'บ้านตัวอย่าง', lat: 15.0, lng: 104.8 }]
const MOCK_VARIETIES: Variety[] = [{ id: 'mock-v1', name: 'ข้าวโพดพันธุ์ A' }]
const MOCK_BOOKINGS: Booking[] = [{ id: 'mock-r1', farmer_id: 'mock-f1', profile_id: 'mock-p1', member_name: 'สมชาย ใจดี', member_phone: '0812345678', variety_id: 'mock-v1', variety_name: 'ข้าวโพดพันธุ์ A', quantity_kg: 10, pickup_date: new Date().toISOString().slice(0, 10), pickup_note: 'รับที่โกดัง', status: 'pending', booked_by_role: 'sales', district: 'สำโรง', village: 'บ้านตัวอย่าง', lat: 15.0, lng: 104.8 }]
...
export default function AdminSeedSales() {
  ...
  const [form, setForm] = useState({ farmer_id: '', variety_id: '', quantity_kg: '', pickup_date: '', pickup_note: '' })
  ...
  const load = async () => {
    ...
    try {
      ...
      const [varietyRes, bookingRes] = await Promise.all([
      ...
      setBookings((bookingRes.data ?? []).map((r: any) => {
        ...
        const m = memberMap.get(farmerId) ?? profileMap.get(profileId)
        return { id: String(r.id), farmer_id: farmerId || m?.id || '', profile_id: profileId || m?.profileId, member_name: String(r.member_name ?? m?.name ?? '-'), member_phone: String(r.member_phone ?? m?.phone ?? ''), variety_id: String(r.variety_id ?? ''), variety_name: String(r.variety_name ?? varietyMap.get(String(r.variety_id ?? '')) ?? '-'), quantity_kg: Number(r.quantity_kg ?? r.quantity ?? 0), pickup_date: String(r.pickup_date ?? ''), pickup_note: String(r.pickup_note ?? ''), status: String(r.status ?? 'pending') as BookingStatus, booked_by_role: String(r.booked_by_role ?? 'sales'), lat: m?.lat, lng: m?.lng, district: m?.district, village: m?.village }
      }))
  ...
  const selectedMember = members.find(m => m.id === form.farmer_id)
  ...
  const create = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = { farmer_id: selectedMember.id, profile_id: selectedMember.profileId ?? null, member_name: selectedMember.name, member_phone: selectedMember.phone, variety_id: selectedVariety.id, variety_name: selectedVariety.name, quantity_kg: qty, quantity: qty, pickup_date: form.pickup_date, pickup_note: form.pickup_note, booked_by: 'admin', booked_by_role: 'sales', status: 'pending' }
      if (!isSupabaseReady || !supabase) setBookings(prev => [{ id: `mock-r-${Date.now()}`, ...payload, profile_id: payload.profile_id ?? undefined, lat: selectedMember.lat, lng: selectedMember.lng, district: selectedMember.district, village: selectedMember.village } as Booking, ...prev])
      else { const { error } = await supabase.from('seed_bookings').insert(payload); if (error) throw new Error(error.message); await load() }
      flash(true, 'บันทึกจองสำเร็จ'); setShowForm(false); setForm({ farmer_id: '', variety_id: '', quantity_kg: '', pickup_date: '', pickup_note: '' })
  ...
  const openMap = (b: Booking) => { if (b.lat && b.lng) window.open(`https://www.google.com/maps?q=${b.lat},${b.lng}`, '_blank'); else flash(false, 'รายการนี้ยังไม่มีพิกัด') }
  const displayed = useMemo(() => bookings.filter(b => filterStatus === 'all' || b.status === filterStatus).filter(b => `${b.member_name} ${b.member_phone} ${b.variety_name} ${b.district} ${b.village}`.toLowerCase().includes(search.toLowerCase())), [bookings, filterStatus, search])
  const total = bookings.length, pending = bookings.filter(b => b.status === 'pending').length, confirmed = bookings.filter(b => b.status === 'confirmed').length, qtyTotal = bookings.filter(b => b.status !== 'cancelled').reduce((s, b) => s + b.quantity_kg, 0)

  return <div className="space-y-5"><div className="flex items-center justify-between flex-wrap gap-3"><div><h1 className="text-xl font-bold text-gray-900">จองเมล็ดพันธุ์</h1><div className="flex items-center gap-1.5 mt-0.5 text-sm">{isSupabaseReady ? <><Wifi className="w-3.5 h-3.5 text-emerald-600"/><span className="text-emerald-600">Farmers + Profiles</span></> : <><WifiOff className="w-3.5 h-3.5 text-amber-500"/><span className="text-amber-600">Mock data</span></>}</div></div><div className="flex gap-2"><button onClick={() => void load()} className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-xl text-sm"><RefreshCw className="w-4 h-4"/>รีโหลด</button><button onClick={() => setShowForm(v => !v)} className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-xl text-sm font-semibold"><Package className="w-4 h-4"/>จองให้สมาชิก</button></div></div>
