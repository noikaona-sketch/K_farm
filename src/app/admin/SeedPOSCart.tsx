  ...
  return (
    <div className="bg-white rounded-2xl border p-4 space-y-4 h-fit sticky top-4">
      ...
      >
        ...
        {farmers.map((f) => (
          <option key={f.id} value={f.id}>{f.name} | {f.phone} | {f.district ?? '-'}</option>
        ))}
      </select>
        ...
        {cart.map((item) => (
          <div key={item.id} className="border rounded-xl p-3">
            <div className="flex justify-between gap-2">
              ...
              <button
                type="button"
                onClick={() => onCartChange(removeCartItem(cart, item.id))}
            ...
            <div className="mt-2 grid grid-cols-3 gap-2 items-center">
              <input
                ...
                max={item.balance}
                value={item.qty}
                onChange={(e) => onCartChange(updateCartQty(cart, item.id, Number(e.target.value)))}
      ...
      <div className="border-t pt-3 space-y-2">
        <div className="flex justify-between text-lg">
          <span>รวม</span>
          <b>{fmtMoney(total)} บาท</b>
        </div>

        <select
          value={paymentType}
          onChange={(e) => onPaymentTypeChange(e.target.value as PosPaymentType)}
          className="w-full border rounded-xl p-2 bg-white"
        >
        ...
        {paymentType === 'cash' ? (
          ...
          <>
            <input
              type="number"
              value={creditPaid}
              onChange={(e) => onCreditPaidChange(e.target.value)}
              placeholder="ชำระแล้วบางส่วน"
              className="w-full border rounded-xl p-2"
            />
            <input
              type="date"
        ...
        <div className="grid grid-cols-2 gap-2">
          <button type="button" onClick={onClear} className="border rounded-xl py-3 font-semibold">
            ล้าง
          </button>
          <button
            type="button"
            ...
            onClick={onSubmit}
            className="bg-emerald-600 disabled:bg-gray-300 text-white rounded-xl py-3 font-bold flex items-center justify-center gap-2"
          >
